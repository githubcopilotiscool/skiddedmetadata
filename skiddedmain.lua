...
local metadata = urlLoad("https://raw.githubusercontent.com/githubcopilotiscool/skiddedmetadata/main/miscellaneous.lua")
...
local SaveManager = {} do
    ...
    function SaveManager:Save(name)
        local fullPath = 'skiddedfriday_autoplayer/configs/' .. name .. '.json'
    ...
    function SaveManager:Load(name)
        local file = 'skiddedfriday_autoplayer/configs/' .. name .. '.json'
    ...
    function SaveManager.Refresh()
        local list = listfiles('skiddedfriday_autoplayer/configs')
...
local Window = UI:CreateWindow({
    Title = string.format('personal autoplayer - version %s | updated: %s', metadata.version, metadata.updated),
    AutoShow = true,
...
Tabs.Main = Window:AddTab('Autoplayer')
Tabs.Miscellaneous = Window:AddTab('Other Stuff')
...
Groups.Credits = Tabs.Miscellaneous:AddRightGroupbox('Credits')
    ...
    addRichText(Groups.Credits:AddLabel('<font color="#de6cff">Zealon</font> - SKIDDER'))
    addRichText(Groups.Credits:AddLabel('<font color="#ff3636">wally</font> - original creator'))
...
Groups.Misc = Tabs.Miscellaneous:AddRightGroupbox('Miscellaneous')
    ...
    Groups.Misc:AddButton('DELETE SCRIPT', function() pcall(shared._unload) end)
    Groups.Misc:AddButton('why did you skid this', function()

    UI:Notify('for fun idrk why', 5)
...
if type(readfile) == 'function' and type(writefile) == 'function' and type(makefolder) == 'function' and type(isfolder) == 'function' then
    makefolder('skiddedfriday_autoplayer')
    makefolder('skiddedfriday_autoplayer\\configs')
...
do
    ...
    themeManager:SetFolder('skiddedfriday_autoplayer')
